...
For example, \"app_url\":\"https://app.testsigma.com/#/projects/31/applications/53/version/72/report/executions/197/runs/819/environments\""
#**********************************
...
function checkTestPlanRunStatus(){
  ...
  do
    ...
    if [[ $EXECUTION_STATUS =~ "STATUS_IN_PROGRESS" ]]; then
      ...
      sleep $SLEEP_TIME
    elif [[ $EXECUTION_STATUS =~ "STATUS_QUEUED" ]]; then
      echo "Poll #$(($i+1)) - Test Execution queued... Wait for $SLEEP_TIME seconds before next poll.."
      sleep $SLEEP_TIME
...
function populateJsonPayload(){
  JSON_DATA='{"environmentId":'$ENVIRONMENT_ID',"executionId":'$TESTSIGMA_TEST_PLAN_ID
  populateRuntimeData
  populateBuildNo
